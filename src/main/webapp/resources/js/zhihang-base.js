...
(function(w, d) {
	...
	ZH = {
		...
		Pop: {
			 ...
			 * @param {Object} title
			 */
			alert: function(icon, message, title) {
				layer.alert(message, {
					icon: ZH.Utils.isNull(icon) ? 1 : icon,
					title: ZH.Utils.isBlank(title) ? '信息' : title
				});
			},
			 ...
			 * 弹出信息框
			 */
			msg: function(icon, message, time) {
				layer.msg(message, {
					icon: ZH.Utils.isNull(icon) ? 1 : icon,
					time: ZH.Utils.isNull(time) ? 3000 : time
				});
			},
			 ...
			 * @param {Object} CanFun
			 */
			confirm: function(content, okFun, CanFun) {
				...
				return layer.confirm(content, {
					...
					if (ZH.Utils.isFunction(okFun)) {
						okFun();
					}
				}, function(index) {
					if (ZH.Utils.isFunction(CanFun)) {
						CanFun();
					}
				});
			},
			 ...
			 * @param {Object} content
			 */
			open: function(title, width, height, content) {
				return layer.open({
					type: 1,
					title: title,
					fix: false, //不固定
		...
		Utils: {
			...
			isBlank: function(str) {
				return str == undefined || str == null || ZH.Utils.trim(str).length == 0;
			},
			/**
			 * 判断方法是否存在
			 * @param {Object} fun
			...
			/**
			 * 格式化时间戳
			 * 由于本方法按照Replace替换进行，所以格式化字符串请尽量标准.
			 * 本方法如果月日时分秒为个数，则将会前面加0
			 * @param {Object} time 时间戳
			 * @param {Object} formatStr 格式化的数据，按照Java时间格式化的规则来，比如：yyyy-MM-dd HH:mm:ss:SSS
			 */
			formatDate: function(time, formatStr) {
				...
				var formatArr = [{
					...
					str: "MM",
					format: (d.getMonth() + 1) < 10 ? "0" + (d.getMonth() + 1) : (d.getMonth() + 1)
				}, {
					str: "dd",
					format: d.getDate() < 10 ? "0" + d.getDate() : d.getDate()
				}, {
					str: "HH",
					format: d.getHours() < 10 ? "0" + d.getHours() : d.getHours()
				}, {
					str: "mm",
					format: d.getMinutes() < 10 ? "0" + d.getMinutes() : d.getMinutes()
				}, {
					str: "ss",
					format: d.getSeconds() < 10 ? "0" + d.getSeconds() : d.getSeconds()
				}, {
					str: "SSS",
					format: d.getMilliseconds() < 10 ? "0" + d.getMilliseconds() : d.getMilliseconds()
				}];

				for (var i = 0; i < formatArr.length; i++) {
					if (formatStr.indexOf(formatArr[i].str) > -1) {
						formatStr = formatStr.replace(formatArr[i].str, formatArr[i].format);
					}
				}
			...
			endWith: function(str, reg) {
				if (reg == null || reg == "" || str.length == 0 || reg.length > str.length)
					return false;
				if (str.substring(str.length - reg.length) == reg)
					return true;
				else
					return false;
				return true;
			...
			startWith: function(str, reg) {
				if (reg == null || reg == "" || str.length == 0 || reg.length > str.length)
					return false;
				if (str.substr(0, reg.length) == reg)
					return true;
				else
					return false;
				return true;
		...
		Ajax: {
			post: function(url, params, ismask, suc, err) {
				...
				$.ajax({
					...
					success:function(response){
						if(!ZH.Utils.isNull(index)){
							ZH.Pop.unmask(index);
						}
						if(response["code"] == 10112) {
							ZH.Pop.alert(5, "您当前没有操作这个功能的权限");
							return;
						}
						if(response["code"] != 1000){
							if(typeof(err) == "function"){
								...
								ZH.Pop.alert(4, response["msg"], null);
							}
							...
							if(typeof(suc) == "function"){
								...
								ZH.Pop.alert(1, response["msg"], null);
							}
						}
					},
					error:function(obj){
			...
			get: function(url, ismask, suc, err) {
				if(ismask){
					index = ZH.Pop.mask();
				}
		...
		Form: {
			loadView: function(parent, module) {
				$.ajax({
					url:"/resources/js/view/" + parent + "/" + module + ".js",
					type:'get',
					dataType:'json',
					success:function(response){
						var html = "";
		...
		Pjax: {
			...
			privateFun: {
				handlerTitle: function() {
					var title = $("head title").text().substr($("head title").text().indexOf(ZH.Pjax.properties.titleSuffix) - 1,
							$("head title").text().length);
					
					$("head title").text($(ZH.Pjax.properties.container + " title").html() + title);
					$(ZH.Pjax.properties.container + " title").remove();
				},
				handlerUrl: function(url, title) {
					if(url.indexOf(ZH.Pjax.properties.pjaxDist) != -1) {
						if(url.charAt(url.indexOf(ZH.Pjax.properties.pjaxDist) - 1) == "?") {
							url = url.replace("?" + ZH.Pjax.properties.pjaxDist, "");
						}else if(url.charAt(url.indexOf(ZH.Pjax.properties.pjaxDist) - 1) == "&"){
							url = url.replace("&" + ZH.Pjax.properties.pjaxDist, "");
						}else {
							url = url.replace(ZH.Pjax.properties.pjaxDist, "");
						}
				...
				handlerLink: function() {
					$("head").append($(ZH.Pjax.properties.container + " link"));
					$(ZH.Pjax.properties.container + " link").remove();
				},
				handlerScript: function() {
					$("body").append($(ZH.Pjax.properties.container + " script"));
					$(ZH.Pjax.properties.container + " script").remove();
				},
				// 对跳转的链接内容中包含框架的进行剥离
				handlerOther: function() {
					$(ZH.Pjax.properties.container + " meta").remove();
					
					/**
					 * 服务端已做Pjax传输判断，返回没有框架，无需进行剥离。
					 */
					/*if($(ZH.Pjax.properties.container).find("div").hasClass("parentWrapper")) {
						$(ZH.Pjax.properties.container).html($(ZH.Pjax.properties.container + " .wrapper-container").html());
					}*/
				}
			},
			loadPjax: function(url, view, err) {
				$.ajax({
					url:url,
					type:'get',
					success:function(response){
						ZH.Pjax.handlerPjax(url, view, response);
					},
					error:function(obj){
						if(typeof(err) == "function"){
							err(obj);
			...
			loadPjaxByForm: function(form, view, err) {
				var src = $(form).attr("action");
				src = src.indexOf("&") != -1 ? src += "&" + ZH.Pjax.properties.pjaxDist : src.indexOf("?") != -1 ? src += "&" + ZH.Pjax.properties.pjaxDist : src += "?" + ZH.Pjax.properties.pjaxDist;
				$.ajax({
					url:src,
					...
					success:function(response,textstatus, xhrReq){
						/**
						 * 处理服务器可能做302跳转，提交url与返回Url不一致的情况
						 */
						if(!ZH.Utils.isNull(xhrReq.getResponseHeader("TM-finalURLdhdg"))) {
							ZH.Pjax.handlerPjax(xhrReq.getResponseHeader("TM-finalURLdhdg"), view, response);
						}else{
							ZH.Pjax.handlerPjax(src, view, response);
						}
					},
					error:function(obj){
						if(typeof(err) == "function"){
			...
			/**
			 * Pjax模式下的window.location.href;
			 */
			reload:function() {
				var src = window.location.href;
				src = src.indexOf("&") != -1 ? src += "&" + ZH.Pjax.properties.pjaxDist : src.indexOf("?") != -1 ? src += "&" + ZH.Pjax.properties.pjaxDist : src += "?" + ZH.Pjax.properties.pjaxDist;
				ZH.Pjax.loadPjax(src, ZH.Pjax.properties.container);
			},
			handlerPjax: function(url, view, context) {
				$(view).html(context);
				
				ZH.Pjax.privateFun.handlerTitle();
				//ZH.Pjax.privateFun.handlerLink();
				//ZH.Pjax.privateFun.handlerScript();
				ZH.Pjax.privateFun.handlerUrl(url, $("head title").text());
				ZH.Pjax.privateFun.handlerOther();
			}
		}
	};
	ZhihangFramen = ZH;
...
/**
 * 现有Pjax库并不适用Framen框架，单独开始开发Pjax框架，
 * 本Pjax框架仅针对a标签的点击事件，判断a标签中参数含有?pjax=true时，对该请求使用pjax方式进行载入
 * @author  cnlyml
 */
$(function(){
	$("body").on("click", "a", function(){
		var src = $(this).attr("href");
		if($(this).data("pjax") == true) {
			src = src.indexOf("&") != -1 ? src += "&" + ZH.Pjax.properties.pjaxDist : src.indexOf("?") != -1 ? src += "&" + ZH.Pjax.properties.pjaxDist : src += "?" + ZH.Pjax.properties.pjaxDist;
			ZH.Pjax.loadPjax(src, ZH.Pjax.properties.container);
			return false;
		}
	...
	$("body").on("submit", "form", function(e) {
		if($(this).data("pjax") == true) {
			ZH.Pjax.loadPjaxByForm($(this), ZH.Pjax.properties.container);
			return false;
		}
	});
